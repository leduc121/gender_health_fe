"use client";
import { MenstrualPrediction } from "@/components/MenstrualCycleDetail";
...
export default function MenstrualTrackerPage() {
  ...
  const [menstrualTrackingConsent, setMenstrualTrackingConsent] =
    useState(false);
  const [userHealthDataConsent, setUserHealthDataConsent] = useState<
    boolean | null
  >(null);
  ...
  const { data: contraceptiveReminders } = useQuery<ContraceptiveReminder[]>({
  ...
  const { mutate: saveReminder, isPending: isSavingReminder } = useMutation({
    mutationFn: (reminderData: {
      id?: string;
      data: CreateContraceptiveReminderDto | UpdateContraceptiveReminderDto;
    }) => {
      if (reminderData.id) {
        return MenstrualService.updateContraceptiveReminder(
          reminderData.id,
          reminderData.data
        );
      }
      return MenstrualService.createContraceptiveReminder(
        reminderData.data as CreateContraceptiveReminderDto
      );
    },
  ...
  // Dialog State
  const [showContraceptiveReminderDialog, setShowContraceptiveReminderDialog] =
    useState(false);
  const [currentReminder, setCurrentReminder] =
    useState<ContraceptiveReminder | null>(null);
  const [newReminderForm, setNewReminderForm] =
    useState<CreateContraceptiveReminderDto>({
      contraceptiveType: "",
      reminderTime: "",
      startDate: "",
      frequency: "daily",
  ...
  useEffect(() => {
    if (user) {
      ...
      if (
        user.gender === "F" &&
        (user.healthDataConsent === null ||
          user.healthDataConsent === undefined)
      ) {
  ...
  useEffect(() => {
    if (cycles && cycles.length > 0) {
      // Check if cycles is not undefined
  ...
  const handleUpdateHealthDataConsent = async (consent: boolean) => {
    try {
      await apiClient.patch(
        API_ENDPOINTS.USERS.PROFILE + "/health-data-consent",
      ...
      toast({
        title: "Lỗi",
        description: e?.message || "Không thể cập nhật quyền.",
        variant: "destructive",
      });
      throw e;
    }
  ...
  const handleCreateCycle = async (e: React.FormEvent) => {
    ...
    if (
      user?.gender === "F" &&
      !userHealthDataConsent &&
      !menstrualTrackingConsent
    ) {
      toast({
        title: "Lỗi",
        description: "Bạn cần đồng ý cho phép thu thập dữ liệu.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleOpenEditReminderDialog = (reminder: ContraceptiveReminder) => {
    ...
    setNewReminderForm({
      ...
      endDate: reminder.endDate
        ? format(new Date(reminder.endDate), "yyyy-MM-dd")
        : undefined,
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <CardContent>
          <form onSubmit={handleCreateCycle} className="space-y-4">
              ...
              <div className="flex items-center space-x-2 mb-4">
                <Checkbox
                  ...
                  onCheckedChange={(checked: boolean) =>
                    setMenstrualTrackingConsent(checked)
                  }
                />
                <Label
            ...
            <Button type="submit" disabled={isCreatingCycle}>
              {isCreatingCycle ? "Đang lưu..." : "Lưu chu kỳ"}
            </Button>
          </form>
        </CardContent>
      </Card>
      <MenstrualCycleHistory
        ...
        isLoading={isLoadingCycles}
        isError={isErrorCycles}
        error={errorCycles as Error | null}
      />
      <Card className="mt-6">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Button onClick={handleOpenCreateReminderDialog}>
            Thêm nhắc nhở
          </Button>
        </CardHeader>
        <CardContent>
                  ...
                  (reminder: ContraceptiveReminder) => (
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex space-x-2">
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              handleOpenEditReminderDialog(reminder)
                            }
                          >
                            Sửa
                          </Button>
                      ...
                      <p className="text-sm text-gray-700">
                        Thời gian: {reminder.reminderTime} | Tần suất:{" "}
                        {reminder.frequency}
                      </p>
                      <p className="text-sm text-gray-700">
                        Từ ngày:{" "}
                        {format(new Date(reminder.startDate), "dd/MM/yyyy")}
                        {reminder.endDate &&
                          ` đến ${format(new Date(reminder.endDate), "dd/MM/yyyy")}`}
                      </p>
      ...
      <Dialog
        open={showContraceptiveReminderDialog}
        onOpenChange={setShowContraceptiveReminderDialog}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {currentReminder ? "Sửa nhắc nhở" : "Thêm nhắc nhở mới"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSaveReminder} className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewReminderForm({
                    ...newReminderForm,
                    contraceptiveType: e.target.value,
                  })
                }
                required
              />
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: "daily" | "weekly" | "monthly") =>
                  setNewReminderForm({ ...newReminderForm, frequency: value })
                }
              >
                <SelectTrigger>
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          const updatedDays = checked
                            ...
                            : (newReminderForm.daysOfWeek || []).filter(
                                (d) => d !== index
                              );
                          setNewReminderForm({
                            ...newReminderForm,
                            daysOfWeek: updatedDays.sort(),
                          });
                        }}
                      />
            ...
            <div>
              <Label htmlFor="reminderMessage">
                Tin nhắn nhắc nhở (Không bắt buộc)
              </Label>
              <Input
                id="reminderMessage"
